...
export default class Hero extends Circle {
  ...
  setCellY(){
	...
	limits(limits){
    if(this.x + this.size*2 >= limits.width && this.status == 'd') {
      this.x = limits.width - this.size*2;
      this.cellX = 0;
    }

    if(this.x < 0 && this.status == 'a') {
      this.x = 0;
      this.cellX = 0;
    }
		
    if(this.y + this.size*2 > limits.height - 2 && this.status == 's'){
      this.y = limits.height - this.size*2 - 2;
      this.cellX = 0;
    }

    if(this.y < 0 && this.status == 'w'){
      this.y = 0;
      this.cellX = 0;
    }	
	}
